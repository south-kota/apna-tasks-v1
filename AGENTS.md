...
The architecture of T3 Code's websocket layer (`apna`) enables a lot of awesome remote features. These have become core to the product. Whether users are connecting directly over their local network, using Tailscale, or leaning in fully with T3 Connect (our tunnel solution, also in this repo), we need to make sure new features are properly supported.

### 4. Multi-surface

T3 Code has 3 key app surfaces: **web**, **desktop**, and **mobile**.

**Web** is kind of two surfaces, as we have the public facing "app.t3.codes" as well as locally hosting the web app through the `apna` command. Both need to be supported by all new features where reasonable.
...
- **Apna home** means the base data directory. Runtime state normally lives below its userdata directory.

## The three ways to hurt yourself

1. **Killing by pattern.** Never `pkill -f`, `pgrep | kill`, or `kill` a PID you found by matching a name, path, or worktree string. Your own agent process has this worktree's path in its argv, and this machine runs several other dev servers at once. Kill only a PID you captured at spawn, or the owner of your port from `ss -H -ltnp` after confirming `/proc/<pid>/cwd` is your worktree.
2. **Writing to the live install.** `~/.apnatasks/userdata` is Kota's real Apna Tasks database, in use while you work. Reading it and copying from it are fine, and a good way to get real test data (see Test data). Never start a server against it, never open it read-write, never clean it up.
...
- `vp run dev` starts server and web. In a worktree, state defaults to that worktree's gitignored `.apnatasks`, which deliberately outranks an ambient `T3CODE_HOME` so you cannot land on shared state by accident. An explicit `--home-dir` still wins.
...
An empty database is a bad test. Seed your worktree's `.apnatasks` with a copy of real data instead of pointing at live state:

- Copy from `~/.apnatasks/userdata` (Kota's real data, the most realistic test set) or `~/.apnatasks/dev`. Worktree state lives at `<worktree>/.apnatasks/userdata`.
- Snapshot the database with `VACUUM INTO`, which is safe even while a server has the source open and yields one consistent file:

  ```bash
  mkdir -p .apnatasks/userdata
  rm -f .apnatasks/userdata/state.sqlite*  # VACUUM INTO refuses to overwrite
  bun -e "new (require('bun:sqlite').Database)(process.env.HOME + '/.apnatasks/userdata/state.sqlite', { readonly: true }).run(\"VACUUM INTO '.apnatasks/userdata/state.sqlite'\")"
  ```
