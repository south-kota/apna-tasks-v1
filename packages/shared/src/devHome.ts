 ...
 * `~/.apnatasks` that a user's installed Apna Tasks runs against.
 *
 * A linked git worktree gets its own (gitignored) `.apnatasks`: feature work in a
...
export const resolveWorktreeT3Home = (
  ...
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, ".apnatasks");
  });
