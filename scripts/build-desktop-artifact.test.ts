...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Apna Tasks (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Apna Tasks (Nightly)");
  });
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      // entry advertises MimeType=x-scheme-handler/apnatasks; for OAuth deep links.
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "Apna Tasks", schemes: ["apnatasks", "apnatasks-dev"] },
      ]);
      assert.equal((linux.linux as Record<string, unknown>).executableName, "apna");
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).desktop, {
        entry: { StartupWMClass: "apnatasks" },
      });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.apnatasks.desktop</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.apnatasks.desktop");
