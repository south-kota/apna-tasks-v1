...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("runDevRunnerWithInput", () => {
    ...
    // Base-dir precedence (--home-dir > worktree .apnatasks > ambient T3CODE_HOME)
    ...
    describe("t3 home precedence", () => {
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".apnatasks"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the worktree .apnatasks over an ambient T3CODE_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
